import { SessionCommand, SessionCommandDeps } from '../types/session-command.js'
...
import { notify } from './session-commands/notify.js'
...
export async function tryToExecuteSessionCommand (
  ...
  commandDeps: SessionCommandDeps,
) {
  ...
  if (commandName && sessionCommands[commandName]) {
    ...
    try {
      await sessionCommands[commandName].execute(message, tokens, session, commandDeps)
      logger.info(
